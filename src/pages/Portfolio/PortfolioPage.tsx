            ...
            {entries.map((transaction: Transaction) => {
              ...
              return (
                ...
                >
                  ...
                  <td
                    className={`${transaction.Name == "Mint" || transaction.Name == "Swap" ? "font-semibold text-red-500" : transaction.Name == "Burn" ? "font-semibold text-green-700" : " "}`}
                  >
                    {`${formatReserve(transaction.AmountToken1)} ${transaction.Token1Symbol}`}
                  </td>
                  <td
                    className={`${transaction.Name == "Mint" ? "font-semibold text-red-500" : transaction.Name == "Burn" || transaction.Name == "Swap" ? "font-semibold text-green-700" : ""}`}
                  >
                    {`${formatReserve(transaction.AmountToken2)} ${transaction.Token2Symbol}`}
                  </td>
                  <td
                    className={`${transaction.Name == "Burn" || transaction.Name == "Swap" ? "font-semibold text-red-500" : "font-semibold text-green-700"}`}
                  >
